...
export async function getPenelitianDosen(page: number, search: string) {
  const response: DosenResponse<PenelitianDosenData> = await getData(API_ENDPOINTS_DOSEN.PENELITIAN, { page, search });
...
export async function getAnggota(page: number, search: string) {
  const response: DosenResponse<DosenData> = await getData(API_ENDPOINTS.ANGGOTA, { page, search });
  return response.data;
}

export async function getListPenelitian() {
  const response: ListPenelitianResponse = await getData(API_ENDPOINTS.LIST_PENELITIAN);
  return response;
}

export async function getListIndeksasi() {
  const response: ListIndeksasiResponse = await getData(API_ENDPOINTS.LIST_INDEKSASI);
  return response;
}

export async function getListPengabdian() {
  const response: ListPengabdianResponse = await getData(API_ENDPOINTS.LIST_PENGABDIAN);
  return response;
}
